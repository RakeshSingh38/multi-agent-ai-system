    ...
    with st.spinner("🔄 Multi-Agent System is researching with REAL DATA..."):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                st.success(f"✅ Research completed! Task ID: {task_id}")
                
                base_url = get_base_url()
                st.markdown(f"🔗 **View API Details:** [{base_url}/tasks/{task_id}]({base_url}/tasks/{task_id})")
                st.info("👉 Check the 'Results' tab to see your analysis!")
            ...
            st.info(f"Make sure the API server is running on port {settings.api_port}")
            cancel_research()


def _show_research_progress(progress_bar, status_text):
    """Show dynamic progress messages during research."""
    messages = [
        "🔍 Research Agent: Gathering data from web sources...",
        "🌐 Research Agent: Searching DuckDuckGo for latest information...",
        "📚 Research Agent: Collecting data from Wikipedia and news feeds...",
        "💰 Research Agent: Analyzing market data and stock prices...",
        "📊 Analysis Agent: Processing market data and insights...",
        "🧠 Analysis Agent: Generating key findings and patterns...",
        "📝 Report Writer: Creating professional analysis report...",
        "✨ Report Writer: Adding executive summary and recommendations...",
